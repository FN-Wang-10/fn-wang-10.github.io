...
document.documentElement.scrollTop——————滚动条距离顶部距离  
document.documentElement.scrollHeight————body 元素高度  
scrollHeight 表示`body`所有元素的总长度(包括 body 元素自身的 padding)  
页面不能滚动时也是存在的,此时 scrollHeight 等于 clientHeigh  
document.documentElement.clientHeight——————可视窗高度
