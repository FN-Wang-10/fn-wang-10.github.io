...
在父组件使用自定义属性传值，在子组件使用 props 接收  
使用插槽（插槽的作用域是父组件的作用域）`<slot></slot>`  
$root,$parent 获取根组件和父组件组件实例  
依赖注入（provide inject）

### 子传父

在父组件里面定义自定义事件，在子组件用 this.$emit()去触发
作用域插槽  `v-slot:default="slotProps"` `<slot :count="count"></slot>`
ref 配合$refs
